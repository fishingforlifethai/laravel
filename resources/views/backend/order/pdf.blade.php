...
<html>
<head>
  <title>รายการคำสั่งซื้อ @if($order)- {{$order->order_number}} @endif</title>
...
<style type="text/css">
  ...
  <div class="invoice-header">
    ...
    <div class="float-right site-address">
      <h4>{{env('APP_NAME')}}</h4>
      <p>{{env('APP_ADDRESS')}}</p>
      <p>เบอร์โทรศัพ: <a href="tel:{{env('APP_PHONE')}}">{{env('APP_PHONE')}}</a></p>
      ...
      <h6>ส่งถึง</h6>
       ...
       <div class="address">
        ...
        <p>
         ...
         <p><strong>เบอร์โทรศัพ:</strong> {{ $order->phone }}</p>
         <p><strong>Email:</strong> {{ $order->email }}</p>
       </div>
    </div>
    <div class="invoice-right-top float-right" class="text-right">
      <h3>หมายเลข #{{$order->order_number}}</h3>
  ...
  <section class="order_details pt-3">
    ...
    <table class="table table-bordered table-stripe">
      <thead>
        <tr>
          <th scope="col" class="col-6">สิ้นค้า</th>
          <th scope="col" class="col-3">จำนวน</th>
          <th scope="col" class="col-3">ราคารวม</th>
        </tr>
      </thead>
      <tbody>
      ...
      @endif --}}
        <tr>
          ...
          @endphp
          <th scope="col" class="text-right ">การขนส่ง:</th>
  ...
  <div class="thanks mt-3">
    <h4>ขอบคุณครับ ขอให้เป็นวันที่ดีนะครับ !!</h4>
  </div>
